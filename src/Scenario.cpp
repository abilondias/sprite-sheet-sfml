...
Game::Scenario::Scenario(const std::string& texturePath, const sf::RenderWindow& window,
                         const std::vector<BoundsCheck> bcs) :
    ...
    height = sprite.getGlobalBounds().size.y;
}
...
float Game::Scenario::getHeight() const { return height; }

void Game::Scenario::setHeight(float v) { height = v; }

void Game::Scenario::draw(sf::RenderTarget& target, sf::RenderStates states) const { target.draw(sprite, states); }

float Game::Scenario::interpolate(float x, float t1, float t2, float s1, float s2) {
    assert(t1 < t2);
